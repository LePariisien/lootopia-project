...
export class TreasureHuntService {
  ...
  getTreasureHunt(token: string, id: string): Observable<any> {
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    });
  ...
  digAHole(token: string, treasureId: string, latitude: number, longitude: number): Observable<any> {
    ...
    const params = { treasureId: treasureId, latitude: latitude.toString(), longitude: longitude.toString() };
    return this.http.get(ApiRoutes.digAHole(), { headers, params });
  }
}
